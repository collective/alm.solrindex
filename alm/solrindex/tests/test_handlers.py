...
class SolrEscapeTests(unittest.TestCase):
    ...
    def test_quotes(self):
        self.assertEqual(self._callFUT('I am "quoted"'), 'I am \\"quoted\\"')
...
class DefaultFieldHandlerTests(unittest.TestCase):
    ...
    def test_escaped_query(self):
        ...
        handler = self._makeOne()
        param = handler.parse_query(field, field_query)
        self.assertEqual(param, {"fq": 'dummyfield:"Hello \\"Solr\\"\\! シ"'})
    ...
    def test_query_multiple_default_operator(self):
        ...
        param = handler.parse_query(field, field_query)
        self.assertEqual(
            param, {"fq": 'dummyfield:("news" OR "sports" OR "\\"local\\"")'}
        )
    ...
    def test_query_multiple_and_operator(self):
        ...
        param = handler.parse_query(field, field_query)
        self.assertEqual(
            param, {"fq": 'dummyfield:("news" AND "sports" AND "\\"local\\"")'}
        )
...
class DateFieldHandlerTests(unittest.TestCase):
    ...
    def test_convert_multiple_dates(self):
        ...
        handler = self._makeOne()
        actual = handler.convert([datetime.date(2009, 9, 8), datetime.date(2009, 9, 9)])
        self.assertEqual(
            actual, ["2009-09-08T00:00:00.000Z", "2009-09-09T00:00:00.000Z"]
        )
    ...
    def test_parse_query_range(self):
        ...
        self.assertEqual(
            query, {"fq": r"dummyfield:[2015\-12\-04T00\:00\:00.000Z TO *]"}
        )

        query = handler.parse_query(field, {"query": date(2015, 12, 4), "range": "max"})
        self.assertEqual(
            query, {"fq": r"dummyfield:[* TO 2015\-12\-04T00\:00\:00.000Z]"}
        )

        query = handler.parse_query(
            field, {"query": [date(2015, 12, 4), date(2015, 12, 5)], "range": "min:max"}
        )
        self.assertEqual(
            query,
            {
...
class TextFieldHandlerTests(unittest.TestCase):
    ...
    def test_complex(self):
        ...
        handler = self._makeOne()
        param = handler.parse_query(field, field_query)
        self.assertEqual(param, {"q": "+dummyfield:((fun OR play) +with Solr^4)"})
